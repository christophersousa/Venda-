...
export function Carrinho() {
  ...
  const { cart, handleRemoveCart } = useContext(Context);
  const {formatMoney} = useCart()

  let value = 0

  cart.forEach((e)=>{ value += e.valor})
            ...
            {cart.map((response, key) => {
              return (
                ...
                >
                  ...
                  <span className="text-center w-1/5 font-semibold text-sm">R$ {formatMoney(response.valor)}</span>
                  <span className="text-center w-1/5 font-semibold text-sm">
                    R$ {formatMoney(response.valor)}
                  </span>
                </div>
              );
            })}
